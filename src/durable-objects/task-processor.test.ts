...
describe('Tool result caching', () => {
  ...
  it('cache miss on different arguments', async () => {
    ...
    vi.mocked(executeTool).mockResolvedValue({ tool_call_id: 'call_1', role: 'tool', content: 'Weather data' });
  ...
  it('mutation tools bypass cache', async () => {
    ...
    vi.mocked(executeTool).mockResolvedValue({ tool_call_id: 'call_1', role: 'tool', content: 'Mutation result' });
  ...
  it('cache stats method returns correct hit/miss counts', async () => {
    const mockState = createMockState();
    const { executeTool } = await import('../openrouter/tools');
