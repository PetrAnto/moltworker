  ...
  private async processTask(request: TaskRequest): Promise<void> {
    ...
    if (this.r2) {
      ...
      if (checkpoint && checkpoint.iterations > 0) {
        ...
        conversationMessages.push({
          role: 'user',
          content: `[SYSTEM RESUME NOTICE] You are resuming from a checkpoint. Your previous work is preserved in this conversation.\n\nRESUME RULES:\n- Continue from the EXACT last known step state — do NOT restart from scratch.\n- Do NOT re-read rules, re-acknowledge the task, or re-discover the repository structure.\n- Trust the preloaded/resume context above — only re-read files when needed for correctness (e.g., to verify a specific change), not to rediscover what you already know.\n- Prioritize unresolved deliverables — check what remains undone and focus on completing it.\n- If you were in the middle of creating files, continue creating them. If you showed "Ready to start", that phase is DONE — proceed to implementation immediately.\n- Do NOT output a new plan or summary of prior work — just continue executing.\n\n${lastActionSummary}`,
        });
        ...
        if (choice.message.tool_calls && choice.message.tool_calls.length > 0) {
          ...
          for (const { toolName, toolResult } of toolResults) {
            ...
            if (validation.isError) {
              ...
              // times then give up with a near-empty response. Inject an explicit hint
              // the first time we see this so the model knows to pivot to a different
              // tool or answer from memory instead of burning iterations on a dead service.
              //
              // Uses role: 'user' with a [SYSTEM] prefix to match the convention used
              // by every other dynamic nudge in this file (identical-error pruning,
              // invalid-args nudge, repeat-tool-call nudge, max-tools nudge). A real
              // role: 'system' message mid-conversation behaves inconsistently across
              // providers — some flatten, some ignore, some honor it. The user-prefixed
              // form is provider-agnostic.
              if (
        ...
        if (hasContent && task.phase === 'work' && task.toolsUsed.length > 0) {
          ...
          if (reviewerAlias) {
            ...
            if (reviewContent) {
              ...
              // the model gave up after a service-unavailable error. Annotate
