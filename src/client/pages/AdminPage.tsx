...
export function AcontextSessionsSection({ loading, configured, items }: AcontextSessionsSectionProps) {
  const formatSessionAge = (isoString: string): string => {
  ...
  const statusClassName = (success: boolean | null): string => {
    if (success === true) return 'success'
    if (success === false) return 'failure'
    return 'unknown'
  }

  const statusSymbol = (success: boolean | null): string => {
  ...
  return (
    <section className="devices-section acontext-section">
      ...
      {configured && items.length > 0 && (
        <div className="acontext-table" role="table" aria-label="Recent Acontext sessions">
          {items.map((session) => (
            <div className="acontext-row" role="row" key={session.id}>
              <span className={`status-chip ${statusClassName(session.success)}`} title="Session outcome">
                {statusSymbol(session.success)}
              </span>
              <span className="session-age" title={session.createdAt}>{formatSessionAge(session.createdAt)}</span>
              <span className="session-model" title={session.model}>{session.model}</span>
              <span className="session-prompt" title={session.prompt}>{truncatePrompt(session.prompt)}</span>
...
export default function AdminPage() {
  ...
  const [acontextSessions, setAcontextSessions] = useState<AcontextSessionInfo[]>([])
  const [acontextConfigured, setAcontextConfigured] = useState(true)
  ...
  const fetchAcontextSessions = useCallback(async () => {
    try {
      const response = await getAcontextSessions()
      setAcontextSessions(response.items)
      setAcontextConfigured(response.configured)
    } catch (err) {
  ...
  return (
    <div className="devices-page">
      ...
      <AcontextSessionsSection
        loading={acontextLoading}
        configured={acontextConfigured}
        items={acontextSessions}
      />
