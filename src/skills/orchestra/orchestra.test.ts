...
describe('buildDraftInitPrompt', () => {
  ...
  it('requires the enriched per-task template fields', () => {
    const prompt = buildDraftInitPrompt({ repo: 'Owner/Repo', modelAlias: 'claude' });
    // All required fields in the task template
...
describe('getPlannerRecommendation', () => {
  it('flags a weak model for escalation', () => {
    // /flash is free, typically below the planner floor
    const rec = getPlannerRecommendation('flash');
    expect(rec.currentAlias).toBe('flash');
    // Below floor → should escalate (unless flash has been enriched IQ>=45)
  ...
  it('returns free and paid candidate lists for button menus', () => {
    ...
    // Candidates must not repeat the current alias
...
- [x] Done task
- [ ] Next task`;
    // Gate is above lastCompletedLine → not "just crossed"
    expect(hasActiveReviewGate(roadmap)).toBe(false);
  });
...
- [x] Done 2`;
  ...
  it('activates the SECOND gate once the first has been crossed', () => {
    // Two gates in one roadmap. The first gate sits behind completed work
    // (gate #1 between task 1 and task 2; both now done). The second gate
    // sits between the most recently completed task and the next pending
    // one, so it must be active. This guards against the regression where
    // hasActiveReviewGate only tracked the first-in-file marker and
    // ignored later gates forever.
    const roadmap = `### Phase 1
- [x] Task 1
...
- [ ] Task 4`;
  ...
  it('does not re-trigger a gate once completed work has passed it', () => {
    // Single gate, but it sits between two completed tasks (already
    // crossed and either reviewed or skipped). Must be inactive even
    // though the marker is still in the file.
...
- [ ] Task 3`;
  ...
  it('does not activate a gate before any completed work exists', () => {
    // Fresh roadmap, nothing completed yet. Review has nothing to review
    // against, so the gate must not trigger on the very first /orch next.
    const roadmap = `<!-- review-gate -->
- [ ] Task 1
- [ ] Task 2`;
    expect(hasActiveReviewGate(roadmap)).toBe(false);
  ...
  it('ignores gates that sit past the next pending task', () => {
    // Gate positioned deep in the roadmap, after the next pending task.
    // The user hasn't reached it yet, so it should not fire now.
    const roadmap = `- [x] Task 1
- [ ] Task 2
<!-- review-gate -->
- [ ] Task 3`;
...
describe('countRecentRiskEscalations', () => {
  ...
  it('counts escalations within the recent terminal-event window', () => {
    // Events are newest-first. windowSize=3 → count escalations that occurred
    // during the last 3 completed tasks (including the in-progress next task,
    // whose terminal has not been logged yet). The escalation after
    // terminal #3 but before terminal #4 is still within the window — it
    // belongs to the task that produced terminal #3.
    // Events past terminal #4 (windowSize exceeded) are excluded.
    const events: OrchestraEvent[] = [
      mkEvent({ eventType: 'runtime_risk_escalation', details: { repo: 'o/r' } }), // in-progress
      mkEvent({ eventType: 'task_complete', details: { repo: 'o/r' } }),            // terminal #1
      mkEvent({ eventType: 'runtime_risk_escalation', details: { repo: 'o/r' } }), // during task #1
      mkEvent({ eventType: 'task_abort', details: { repo: 'o/r' } }),               // terminal #2
      mkEvent({ eventType: 'task_complete', details: { repo: 'o/r' } }),            // terminal #3
      mkEvent({ eventType: 'runtime_risk_escalation', details: { repo: 'o/r' } }), // during task #3
      mkEvent({ eventType: 'task_complete', details: { repo: 'o/r' } }),            // terminal #4 — EXCEEDS window
    ];
  ...
  it('REVIEW_RISK_THRESHOLD is a small positive integer', () => {
    // Guard against accidental renames/zeroing; threshold semantics matter.
