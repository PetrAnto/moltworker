...
describe('saved files security', () => {
  ...
  it('sanitizes null bytes in delete name', async () => {
    const deleteFile = vi.fn().mockResolvedValue({ success: true });
    ...
    expect(result.content).toBe('File deleted: badname.txt');
    expect(deleteFile).toHaveBeenCalledWith({ sessionId: 'default', name: 'badname.txt' });
  });

  it('sanitizes control characters in file names', async () => {
    const listFiles = vi.fn().mockResolvedValue([]);
