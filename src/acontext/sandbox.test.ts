...
describe('run_code tool', () => {
  ...
  it('repairs malformed JSON arguments before execution', async () => {
    const executeCode = vi.fn().mockResolvedValue({ stdout: 'ok', stderr: '', exitCode: 0, executionTimeMs: 1 });
    ...
    const result = await executeTool({
      ...
      function: {
        name: 'run_code',
        arguments: "{language: 'python', code: 'print(1)', timeout: 10,}",
      },
    }, { acontextClient: { executeCode } as unknown as AcontextClient });

    expect(result.content).toBe('ok');
    expect(executeCode).toHaveBeenCalledWith(expect.objectContaining({ language: 'python', timeout: 10 }));
  });
});
