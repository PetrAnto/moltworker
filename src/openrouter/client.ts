...
export async function parseSSEStream(
  ...
  try {
    while (true) {
      ...
      for (const part of parts) {
        ...
        try {
          ...
          // Detect provider error payloads embedded in SSE data chunks.
          // Some providers (DashScope, Moonshot, DeepSeek) emit errors as
          // SSE data rather than HTTP status codes.
          if (chunk.error) {
            const providerError = typeof chunk.error === 'string'
              ? chunk.error
              : (() => {
                  if (!chunk.error || typeof chunk.error !== 'object') return JSON.stringify(chunk.error);
                  const details = chunk.error as { message?: unknown; code?: unknown; type?: unknown; error_msg?: unknown };
                  const message = typeof details.message === 'string' ? details.message
                    : typeof details.error_msg === 'string' ? details.error_msg
    ...
    // Flush trailing bytes: some providers close without a final \n
