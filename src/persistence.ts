...
/**
 * Extended Sandbox interface with backup/restore methods.
 * These methods exist on newer versions of the Sandbox SDK but may not
 * be reflected in the type definitions yet.
 */
interface SandboxWithBackup extends Sandbox {
  restoreBackup(handle: { id: string; dir: string }): Promise<void>;
  createBackup(options: { dir: string; ttl: number }): Promise<{ id: string; dir: string }>;
}
 ...
 */
export async function restoreIfNeeded(sandbox: Sandbox | SandboxWithBackup, bucket: R2Bucket): Promise<void> {
  if (restored) {
  ...
  const t0 = Date.now();
  try {
    await (sandbox as SandboxWithBackup).restoreBackup(handle);
...
export async function createSnapshot(
  sandbox: Sandbox | SandboxWithBackup,
  bucket: R2Bucket,
  ...
  const t0 = Date.now();
  const handle = await (sandbox as SandboxWithBackup).createBackup({
    dir: BACKUP_DIR,
    ttl: 604800, // 7 days
  });
