...
// GET /api/admin/acontext/sessions - List recent Acontext sessions for dashboard links
adminApi.get('/acontext/sessions', async (c) => {
  const acontextClient = createAcontextClient(c.env.ACONTEXT_API_KEY, c.env.ACONTEXT_BASE_URL);
  ...
  try {
    const sessions = await acontextClient.listSessions({ limit: 10, timeDesc: true });

    return c.json({
      items: sessions.items.map((session) => {
        const configs = session.configs as Record<string, unknown> | null;
        const prompt = configs?.taskPrompt;
        const success = configs?.success;
        const model = configs?.model;
        const toolsUsed = configs?.toolsUsed;

        return {
          id: session.id,
          model: typeof model === 'string' && model.length > 0 ? model : 'unknown',
          prompt: typeof prompt === 'string' ? prompt : '',
          toolsUsed: typeof toolsUsed === 'number' ? toolsUsed : 0,
